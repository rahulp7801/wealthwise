...
const ProfitMarginComponent2 = () => {
  ...
  const fetchFinancialData = async () => {
    try {
      ...
      if (data && data.annualReports && data.annualReports.length > 0) {
        const { netIncome, totalRevenue } = data.annualReports[1];
  ...
  return (
    <div>
      {profitMargin !== null ? (
        <p>Profit Margin 2021: {profitMargin.toFixed(2)}%</p>
      ) : (
        <p>Loading...</p>
      )}
    </div>
...
export default ProfitMarginComponent2;
